  ...
  if(req.method==="GET"){
    try{
      ...
      if(resource==="analysis"){
        const data=await rest("stock_analyses?user_id=eq."+uid+"&select=*,instruments(market,ticker,name,sector,industry)&order=analysis_date.desc,created_at.desc");
        const rows=data.map((r:any)=>({...r,market:r.instruments?.market,ticker:r.instruments?.ticker,name:r.instruments?.name,sector:r.instruments?.sector,industry:r.instruments?.industry,instruments:undefined}));
  ...
  if(req.method==="POST"){
    ...
    try{
      ...
      if(resource==="research"){
        await rest("research_notes?user_id=eq."+uid,{method:"DELETE"});const payload=rows.map((r:any)=>({user_id:uid,written_at:r.date||r.written_at||new Date().toISOString().slice(0,10),note_type:r.type||r.note_type||null,target:r.target||null,title:r.title||null,fact:r.fact||null,interpretation:r.interpretation||null,source:r.source||null,source_type:r.source_type||null,verified:r.verified??null,market_impact:r.market_impact||null,related_assets:r.related_assets||null,importance:r.importance===""?null:r.importance,next_review_date:r.next_review_date||null,status:r.status||null}));
      ...
      if(resource==="analysis"){
        await rest("stock_analyses?user_id=eq."+uid,{method:"DELETE"});const payload=rows.map((r:any)=>{const inst=resolve(r);if(!inst)return null;return{user_id:uid,instrument_id:inst.id,analysis_date:r.date||r.analysis_date||new Date().toISOString().slice(0,10),eps_growth_q:r.eps_growth_q===""?null:r.eps_growth_q,sales_growth_q:r.sales_growth_q===""?null:r.sales_growth_q,eps_growth_3y:r.eps_growth_3y===""?null:r.eps_growth_3y,roe:r.roe===""?null:r.roe,operating_margin:r.operating_margin===""?null:r.operating_margin,debt_ratio:r.debt_ratio===""?null:r.debt_ratio,operating_cashflow_positive:r.operating_cashflow_positive===""?null:r.operating_cashflow_positive,pe:r.pe===""?null:r.pe,peg:r.peg===""?null:r.peg,moat:r.moat||null,growth_driver:r.growth_driver||null,key_risk:r.key_risk||null,auto_grade:r.auto_grade||null,conclusion:r.conclusion||null}}).filter(Boolean);
