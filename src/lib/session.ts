...
export type WorkspaceResource = 'watchlist' | 'portfolio' | 'research' | 'journal'
...
export function login(email: string, password: string) {
  return authRequest({ action: 'login', email, password })
}

export function signup(email: string, password: string, setupCode: string) {
  return authRequest({ action: 'signup', email, password, setup_code: setupCode })
}

export function refresh(session: Session) {
  return authRequest({ action: 'refresh', refresh_token: session.refresh_token })
}

export async function ensureSession(session: Session): Promise<Session> {
  const expiresAt = Number(session.expires_at || 0)
  if (!expiresAt || Date.now() / 1000 < expiresAt - 90) return session
...
export async function loadWorkspace(session: Session, resource: WorkspaceResource) {
  ...
  const res = await fetch(BASE + '/workspace?resource=' + resource, {
    headers: { Authorization: 'Bearer ' + active.access_token }
  })
  const payload = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(String(payload?.error || 'workspace_read_failed'))
...
export async function saveWorkspace(session: Session, resource: WorkspaceResource, rows: any[]) {
  ...
  const payload = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(String(payload?.error || 'workspace_write_failed'))
